...
const legacyStaticPagePath = path.resolve(process.cwd(), 'src/components/LegacyStaticPage.tsx')
const legacyStaticPageSource = await readFile(legacyStaticPagePath, 'utf8')

const responsiveStylesMatch = legacyStaticPageSource.match(
  /const responsiveFallbackStyles = `([\s\S]*?)`\n\s*const mobileNavScript = `/,
)
const mobileScriptMatch = legacyStaticPageSource.match(/const mobileNavScript = `([\s\S]*?)`\n\n\s*return \(/)

if (!responsiveStylesMatch || !mobileScriptMatch) {
  throw new Error('Failed to extract responsive styles or mobile nav script from LegacyStaticPage.tsx')
}

const responsiveFallbackStyles = responsiveStylesMatch[1]
const mobileNavScript = mobileScriptMatch[1]
